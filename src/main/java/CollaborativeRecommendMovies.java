...
public class CollaborativeRecommendMovies {
	...
	public static void main(String[] args) {
		...
		movieDS.createOrReplaceTempView("movies");
	    	    ...
	    	    // $example on$
		JavaRDD<RatingVO> ratingsRDD = spark.read().textFile("data/movie/u.data").javaRDD()
				.map(row -> {
		...
		spark.sql("select m.movieTitle,p.* from predictions p,movies m " +
				"where p.userId = 633 and p.movieId = m.movieId order by p.prediction desc").show();
		predictions.show();
